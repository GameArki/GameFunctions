...
namespace RVO {
    ...
    internal class KdTree {
         ...
         */
        internal void buildObstacleTree(IList<Obstacle> simuObstacles) {
            obstacleTree_ = new ObstacleTreeNode();

            IList<Obstacle> obstacles = new List<Obstacle>(simuObstacles.Count);

            for (int i = 0; i < simuObstacles.Count; ++i) {
                obstacles.Add(simuObstacles[i]);
            }

            obstacleTree_ = buildObstacleTreeRecursive(obstacles, simuObstacles);
        }
         ...
         */
        private ObstacleTreeNode buildObstacleTreeRecursive(IList<Obstacle> obstacles, IList<Obstacle> simuObstacles) {
...

            {
                ...
                for (int j = 0; j < obstacles.Count; ++j) {
                    ...
                    if (j1LeftOfI >= -RVOMath.RVO_EPSILON && j2LeftOfI >= -RVOMath.RVO_EPSILON) {
                        ...
                        newObstacle.id_ = simuObstacles.Count;

                        simuObstacles.Add(newObstacle);
                ...
                node.left_ = buildObstacleTreeRecursive(leftObstacles, simuObstacles);
                node.right_ = buildObstacleTreeRecursive(rightObstacles, simuObstacles);
